...
#include "ui_zGreenPayCoincontroldialog.h"
...
std::list<std::string> ZGreenPayCoinControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZGreenPayCoinControlDialog::listMints;

ZGreenPayCoinControlDialog::ZGreenPayCoinControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZGreenPayCoinControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZGreenPayCoinControlDialog::~ZGreenPayCoinControlDialog()
{
    delete ui;
}

void ZGreenPayCoinControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZGreenPayCoinControlDialog::updateList()
{
...
void ZGreenPayCoinControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZGreenPayCoinControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZGreenPayCoin_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZGreenPayCoinControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZGreenPayCoinControlDialog::GetSelectedMints()
{
...
void ZGreenPayCoinControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
