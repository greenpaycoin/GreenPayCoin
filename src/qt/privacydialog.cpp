...
#include "zGreenPayCoincontroldialog.h"
#include "spork.h"
                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zgpc ought to be enough for anybody." - Bill Gates, 2017
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ...
        ui->pushButtonMintzsnd->setToolTip(tr("zgpc is currently disabled due to maintenance."));

        ui->pushButtonSpendzsnd->setEnabled(false);
        ui->pushButtonSpendzsnd->setToolTip(tr("zgpc is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_pushButtonMintzsnd_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zgpc is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zgpc...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zgpc in ") +
...
void PrivacyDialog::on_pushButtonSpendzsnd_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zgpc is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zgpc
        sendzsnd();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zgpc
    sendzsnd();
}

void PrivacyDialog::on_pushButtonZGreenPayCoinControl_clicked()
{
    ZGreenPayCoinControlDialog* zsndControl = new ZGreenPayCoinControlDialog(this);
    zsndControl->setModel(walletModel);
    zsndControl->exec();
}

void PrivacyDialog::setZGreenPayCoinControlLabels(int64_t nAmount, int nQuantity)
{
...
void PrivacyDialog::sendzsnd()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid GreenPayCoin Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    // Warn for additional fees if amount is not an integer and change as zgpc is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " GreenPayCoin </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zgpc</b>";
    ...
    // use mints from zgpc selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZGreenPayCoinControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZGreenPayCoinControlDialog::GetSelectedMints();
    }
    ...
    // Clear zGreenPayCoin selector in case it was used
    ZGreenPayCoinControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zgpc Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " GreenPayCoin, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zgpc Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zgpc </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zgpc "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zgpc "));
