...
// Copyright (c) 2017 The GreenPayCoin developers
// Distributed under the MIT software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#if defined(HAVE_CONFIG_H)
#include "config/GreenPayCoin-config.h"
#endif
...
void PrepareShutdown()
{
    ...
    RenameThread("GreenPayCoin-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "GreenPayCoin.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "GreenPayCoind.pid"));
#endif
    strUsage += HelpMessageOpt("-reindex", _("Rebuild block chain index from current blk000??.dat files") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexaccumulators", _("Reindex the accumulator database") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the GreenPayCoin and zgpc money supply statistics") + " " + _("on startup"));
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 29721, 5520));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in GreenPayCoin/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in GreenPayCoin/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, GreenPayCoin, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in GreenPayCoin/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all GreenPayCoin specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-masternodeaddr=<n>", strprintf(_("Set external address:port to get to this masternode (example: %s)"), "128.127.106.235:29721"));
    ...
    strUsage += HelpMessageOpt("-backupzGreenPayCoin=<n>", strprintf(_("Enable automatic wallet backups triggered after each zgpc minting (0-1, default: %u)"), 1));

//    strUsage += "  -anonymizeGreenPayCoinamount=<n>     " + strprintf(_("Keep N GreenPayCoin anonymized (default: %u)"), 0) + "\n";
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2017-%i The GreenPayCoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
{
    RenameThread("GreenPayCoin-loadblk");
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                // GreenPayCoin: load previous sessions sporks if we have them.
                ...
                if (GetBoolArg("-reindexmoneysupply", false)) {
                    if (chainActive.Height() >= Params().Zerocoin_AccumulatorStartHeight()) {
                        RecalculateZGreenPayCoinMinted();
                        RecalculateZGreenPayCoinSpent();
                    }
                    RecalculateGreenPayCoinSupply(1);
                }
                ...
                // GreenPayCoin: recalculate Accumulator Checkpoints that failed to database properly
...
// ********************************************************* Step 8: load wallet
    ...
    if (fDisableWallet) {
        ...
        bool fEnableZGreenPayCoinBackups = GetBoolArg("-backupzGreenPayCoin", true);
        pwalletMain->setZGreenPayCoinAutoBackups(fEnableZGreenPayCoinBackups);
    }  // (!fDisableWallet)
#else  // ENABLE_WALLET
...
// XX42 Remove/refactor code below. Until then provide safe defaults
    nAnonymizeGreenPayCoinAmount = 2;
...
//    }
//
//    nAnonymizeGreenPayCoinAmount = GetArg("-anonymizeGreenPayCoinamount", 0);
...
//    if (nAnonymizeGreenPayCoinAmount < 2) nAnonymizeGreenPayCoinAmount = 2;
    ...
    /* Denominations
       ...
       For example:
       1GreenPayCoin+1000 == (.1GreenPayCoin+100)*10
       10GreenPayCoin+10000 == (1GreenPayCoin+1000)*10
    */
